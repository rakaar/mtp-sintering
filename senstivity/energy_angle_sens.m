...
ord = 1;
% sensitivity
param_range = linspace(2.3*0.01,2.7*0.01,10);
param_range2 = linspace(142*(pi/180),148*(pi/180),10);
...
for param=1:length(param_range)
    for param2=1:length(param_range2)
        surface_energy = param_range(param);
        dihedral_angle = param_range2(param2);
        t_regime_change = 0;
        for t=2:length(t_durn)
            ...
            if d < R/5
                ...
                force_at_single_t(param, param2) = sqrt(force_total_x(t)^2 + force_total_y(t)^2);
            end % d > r/2
